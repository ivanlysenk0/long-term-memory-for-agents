#!/usr/bin/env python3
"""
ltm_doctor.py - health check for the agent long-term memory (LTM vault).

Cross-platform port of the macOS scripts lint.sh + concept_scout.sh.
Behaves the same on Ubuntu, macOS and Windows. No dependencies, stdlib only.

Usage:
    python3 ltm_doctor.py                 check the vault (path taken from the environment)
    python3 ltm_doctor.py --vault PATH    pass the path explicitly
    python3 ltm_doctor.py --scout         only scan for concept candidates in knowledge/
    python3 ltm_doctor.py --all           scout, then the full check
    python3 ltm_doctor.py --json          machine-readable output for an agent
    python3 ltm_doctor.py --quiet         summary only

The vault path is resolved in this order: --vault, the LTM_VAULT variable,
the .ltm-vault file next to the script, the current directory.

Exit code: 0 if there are no errors (warnings are allowed), 1 if there are errors.
"""
...
# Файлы *.seed.md кладёт ltm_seed.py, когда такая страница уже есть в памяти.
# Это временные файлы на разбор человеком, а не часть графа: ругаться на них
# как на сироты и заготовки бессмысленно, это шум, в котором теряются
# настоящие ошибки.
SEED_SUFFIX = ".seed.md"
...
TRIGGERS = [
    r"deep analysis", r"detailed breakdown", r"architecture", r"pattern",
    r"root cause", r"decision made", r"decided to", r"comparison",
    r"conclusion", r"reference implementation", r"canonical",
    r"trade-off", r"tradeoff", r"post-mortem", r"lessons learned",
    r"design choice", r"rationale", r"benchmark", r"why we",
]
...
def clean_link(link: str) -> str:
    """Extract the bare path from [[path\\|label#anchor]].

    Obsidian requires escaping | inside tables, so the backslash is stripped."""
    return link.replace("\\|", "|").split("|")[0].split("#")[0].strip()


def split_frontmatter(text: str):
    """Return (dict of fields, body). If there is no frontmatter, return (None, text)."""
...
def find_vault(explicit: str | None) -> Path:
    ...
    for c in candidates:
        ...
        if c.is_dir():
            return c.resolve()
    raise SystemExit(
        "Vault not found. Pass the path with --vault /path/to/vault, "
        "or set the LTM_VAULT variable, or put the path into a .ltm-vault file next to the script."
    )
...
def collect_md(vault: Path) -> list[Path]:
    ...
    for p in vault.rglob("*.md"):
        if SKIP_DIR_PARTS & set(p.relative_to(vault).parts[:-1]):
            continue
        # Временные файлы слияния seed: не часть графа, в проверки не берутся.
        if is_seed_file(p):
            continue
        if p.is_symlink():
            continue
...
def run_checks(vault: Path, files: list[Path], rep: Report) -> None:
    ...
    for f in files:
        ...
        fm, _ = parsed[f]
        if fm is None:
            rep.error("frontmatter", "no YAML frontmatter", rel(f))
            no_fm += 1
            continue
        for field in REQUIRED_FIELDS:
            if field not in fm:
                rep.error("fields", f"missing field '{field}'", rel(f))
                missing_fields += 1
    ...
    for f in files:
        ...
        if d and not DATE_RE.match(d.strip().strip('"').strip("'")):
            rep.error("date", f"date is not in YYYY-MM-DD format: '{d}'", rel(f))
            bad_dates += 1
        ...
        for link in WIKILINK_RE.findall(text):
            ...
            if "/" in target:
                if target in rel_paths or (vault / target).exists() or (vault / f"{target}.md").exists():
                    continue
                rep.warn("links", f"broken wiki-link [[{target}]]", rel(f))
                broken += 1
                continue
    ...
    for f in files:
        ...
        for link in WIKILINK_RE.findall(read_text(f)):
            ...
            if target.startswith("../") or target.startswith("./"):
                rep.warn("links", f"relative path in wiki-link [[{target}]]: "
                                  "Obsidian does not resolve them, write the path from the memory root", rel(f))
                relative_links += 1
                continue
            ...
            if real:
                rep.warn("links", f"wrong project name in wiki-link [[{target}]]: "
                                  f"the directory is called '{real}', not '{first}'", rel(f))
                wrong_case += 1
    ...
    for name, paths in dupes.items():
        if len(paths) > 1:
            rep.warn("duplicates", f"duplicate knowledge page name: {name} ({len(paths)} files)",
                     "; ".join(paths))
            dupe_count += 1
    ...
    for f in files:
        ...
        if not SESSION_RE.match(f.name):
            rep.warn("sessions", "name is not in YYYY-MM-DD_HHMM_agent_topic.md format", rel(f))
            bad_sessions += 1
    ...
    for f in files:
        ...
        if inbound.get(f.stem, 0) == 0:
            rep.warn("orphans", "no inbound wiki-links", rel(f))
            orphans += 1
    ...
    for f in files:
        ...
        if 0 < words < STUB_WORDS:
            rep.warn("stubs", f"stub, {words} words", rel(f))
            stubs += 1
    ...
    for pf in vault.rglob("pending-concepts.md"):
        ...
        for b in blocks:
            ...
            rep.warn("pending", f"not compiled into knowledge/: {session}", rel(pf))
            pending += 1
...
def run_scout(vault: Path, recent_hours: int | None, verbose: bool = True) -> list[dict]:
    """Flag session logs that are ready to be compiled into knowledge/."""
    queued = []
    ...
    for sessions_dir in sorted(vault.glob("*/sessions")):
        ...
        for sf in sorted(sessions_dir.glob("*.md")):
            ...
            if verbose:
                print(f"QUEUED: {sf.stem} ({len(matched)} triggers) - {project}")
    return queued
...
def _append_pending(pending: Path, project: str, session: Path, vault: Path, matched: list[str]) -> None:
    if not pending.is_file():
        ...
        pending.write_text(
            "---\n"
            'title: "Queue of session logs to compile into knowledge/"\n'
            f"date: {today}\nproject: {project}\nagent: ltm-doctor\ntype: pattern\n"
            "tags:\n  - karpathy\n  - automation\n  - concept-scout\n"
            "status: active\nsources: 0\n---\n\n"
            "# Pending concepts queue\n\n"
            "> Filled in by ltm_doctor --scout. Processed by the agent.\n\n"
            "## Active queue\n",
            encoding="utf-8",
        )
    ...
    with pending.open("a", encoding="utf-8") as fh:
        fh.write(
            ...
            f"- Status: pending\n"
            f"- Suggested action: extract the concept(s) into knowledge/<category>/, update index.md and log.md\n"
        )
...
def main() -> int:
    ap = argparse.ArgumentParser(description="Health check for the agent long-term memory")
    ap.add_argument("--vault", help="path to the vault")
    ap.add_argument("--scout", action="store_true", help="only scan for concept candidates")
    ap.add_argument("--all", action="store_true", help="scout, then the full check")
    ap.add_argument("--recent", type=int, metavar="HOURS", help="scout only recent sessions")
    ...
    if not args.quiet:
        for item in rep.errors:
            print(f"{c.red}ERROR{c.off} [{item['check']}] {item['message']}  {c.dim}{item['file']}{c.off}")
        for item in rep.warnings:
            print(f"{c.yellow}WARNING{c.off} [{item['check']}] {item['message']}  {c.dim}{item['file']}{c.off}")
    ...
    labels = [
        ...
        ("bad_dates", "invalid date"),
        ("broken_links", "broken wiki-links"),
        ("duplicate_names", "duplicate names"),
        ("bad_session_names", "session names off format"),
        ("orphans", "orphan pages"),
        ("stubs", "stubs"),
        ("pending_concepts", "waiting to be compiled"),
    ]
    print("--- Summary ---")
    for key, label in labels:
        print(f"  {label}: {rep.stats[key]}")
    print(f"\nErrors: {c.red}{len(rep.errors)}{c.off}   Warnings: {c.yellow}{len(rep.warnings)}{c.off}")

    if not rep.errors and not rep.warnings:
        print(f"{c.green}Memory is healthy.{c.off}")
        return 0
    if not rep.errors:
        print(f"{c.yellow}No errors, some warnings.{c.off}")
        return 0
    print(f"{c.red}There are errors, the memory needs fixing.{c.off}")
    return 1
